...
- 事実ラベルは allowed_terms から選ぶ。加えて、事実を増やさない短い構造語（原因/結果/依存/対立/変化/支援/圧力/影響/流れ）は使ってよい
...
def _fallback_diagram_blueprint(row: dict, allowed_terms: Optional[list] = None) -> dict:
    ...
    terms = _limit_allowed_terms(
        (allowed_terms or []) + _auto_extract_terms(sent) + DIAGRAM_CONNECTOR_TERMS,
        sent,
...
def _normalize_diagram_blueprint(value, row: dict, allowed_terms: Optional[list] = None) -> dict:
    ...
    sent = row.get("sentence", "")
    terms = _limit_allowed_terms(
        list(value.get("labels") or []) + list(allowed_terms or []) + _auto_extract_terms(sent) + DIAGRAM_CONNECTOR_TERMS,
        sent,
        allow_connectors=True,
    )
...
def _blueprint_prompt_fragment(blueprint: dict) -> str:
    ...
    return (
        ...
        "short structural connector labels are allowed when they do not add new facts; "
        f"avoid = {forbidden}."
    )
...
def _fallback_prompt_for_row(row: dict) -> dict:
    ...
    auto_terms = _limit_allowed_terms(
        _auto_extract_terms(sent) + (DIAGRAM_CONNECTOR_TERMS if row_type == "diagram" else []),
        sent,
        allow_connectors=row_type == "diagram",
    )
    text_rule = (
...
{{
  ...
  "labels": ["画像内に置く短い日本語ラベル。allowed_terms または許可された構造語だけ"],
...
diagram の画像内ラベルは、対象 sentence 由来の allowed_terms に加えて、事実を増やさない構造語（原因/結果/背景/変化/影響/依存/支援/圧力/対立/比較/流れ/転換/選択肢/支配/統治/供給/需要/制裁/回避/現在/過去）だけ使ってよい。
...
3. **画像内テキスト**: 固有名詞・国名・人物名・数値などの事実ラベルは allowed_terms のみ。diagram では事実を増やさない短い構造語だけ追加可
        ...
        if no in prompts_by_no:
            ...
            for t in merged_terms:
                if not isinstance(t, str):
                    continue
                t = t.strip()
                is_connector = p.get("type") == "diagram" and t in DIAGRAM_CONNECTOR_TERMS
                if t and (t in sent or is_connector) and t not in seen:
                    seen.add(t)
                    verified.append(t)
            ...
            p["allowed_terms"] = _limit_allowed_terms(
                verified + (DIAGRAM_CONNECTOR_TERMS if p["type"] == "diagram" else []),
                sent,
                allow_connectors=p["type"] == "diagram",
            )
            ...
            fallback_terms = _limit_allowed_terms(
                auto_terms + (DIAGRAM_CONNECTOR_TERMS if row_type == "diagram" else []),
                sent,
                allow_connectors=row_type == "diagram",
            )
            fallback_text_rule = (
                f"Use these Japanese labels and only fact-safe connector words: {', '.join(fallback_terms)}. Do NOT add any new factual names, numbers, or claims."
